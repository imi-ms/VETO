<template>
  ...
  <!-- View -->
  <div id="vetoView" class="h-100 d-flex flex-column" style="height: 100%">

    <!-- Header -->
    <div class="border-bottom border-2 rounded-top veto-view-header">
      ...
      <div class="veto-container-row">
        ...
        <div class="veto-container-item veto-input">
          <label for="textAlignment" class="form-label veto-header-icon-label">
            <font-awesome-icon icon="fa-align-center" class="veto-header-icon"
                               v-if="textAlignment == 'center'"></font-awesome-icon>
            <font-awesome-icon icon="fa-align-justify" class="veto-header-icon"
                               v-if="textAlignment == 'justify'"></font-awesome-icon>
            <font-awesome-icon icon="fa-align-left" class="veto-header-icon"
                               v-if="textAlignment == 'left'"></font-awesome-icon>
            <font-awesome-icon icon="fa-align-right" class="veto-header-icon"
        ...
        <span class="veto-container-item veto-input" title="Line gap">
          <label for="lineGap" class="form-label veto-header-icon-label">
            <font-awesome-icon icon="fa-arrows-left-right-to-line" rotation="90"
                               class="veto-header-icon"></font-awesome-icon>
          </label>
        ...
        <span class="veto-container-item veto-input" title="Font size">
          <label for="fontSize" class="form-label veto-header-icon-label">
            <font-awesome-icon icon="fa-text-height" class="veto-header-icon"></font-awesome-icon>
          </label>
          <span class="veto-size-25">
    ...
    <!-- Text -->
    <div class="overflow-auto p-2 flex-fill veto-text-surface" @mouseup="textSelected($event)" :style="{'text-align': textAlignment + ' !important'}">
...
export default defineComponent({
  ...
  methods: {
    ...
    openTag(color: string, index: string, size: number, part: Part, classNames: string = ''): string {
      ...
      const padding = ((size + 1) * 0.30) - 0.15;

      let classStyle;
      switch (part) {
        case 'complete':
          classStyle = `veto-highlight-rounded`
          break;
        case 'left':
          classStyle = `veto-highlight-rounded-start`
          break;
        case 'center':
          classStyle = ``
          break;
        case 'right':
          classStyle = `veto-highlight-rounded-end`
          break;
        default:
          break;
      }
...
#vetoView {
  border: 2px solid color-mix(in srgb, var(--veto-color-border) 82%, transparent);
  border-radius: var(--veto-radius-md) !important;
  box-shadow: none;
  overflow: hidden;
}
...
.veto-text-surface {
  background: var(--veto-color-background-highlight);
  border-top: 1px solid var(--veto-color-border);
}
...
.veto-header-icon-label {
  align-items: center;
  display: inline-flex;
  height: var(--veto-size-main);
  margin-right: 0.3rem;
  margin-bottom: 0;
}
...
:deep(.veto-highlight-rounded) {
  border-radius: 0.28rem;
}

:deep(.veto-highlight-rounded-start) {
  border-bottom-left-radius: 0.28rem;
  border-top-left-radius: 0.28rem;
}

:deep(.veto-highlight-rounded-end) {
  border-bottom-right-radius: 0.28rem;
  border-top-right-radius: 0.28rem;
}

:deep(.small-font .veto-highlight-rounded) {
  border-radius: 0.16rem;
}

:deep(.small-font .veto-highlight-rounded-start) {
  border-bottom-left-radius: 0.16rem;
  border-top-left-radius: 0.16rem;
}

:deep(.small-font .veto-highlight-rounded-end) {
